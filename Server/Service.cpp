...
void Service::process(int argc, TCHAR* argv[]) {
	...
	SERVICE_TABLE_ENTRY DispatchTable[] = {
		{(TCHAR*)SVCNAME,(LPSERVICE_MAIN_FUNCTION)Service::serviceMain},
		{NULL,NULL}
	};
	startService();
...
void Service::serviceMain() {
	gSvcStatusHandle = RegisterServiceCtrlHandler(SVCNAME, (LPHANDLER_FUNCTION)Service::svcCtrlHandler);
	if (!gSvcStatusHandle) {
...
void Service::createRegistryRecord() {
	...
	Ret =
		RegCreateKeyEx(
			HKEY_LOCAL_MACHINE,
			TEXT("SOFTWARE\\Anti-virus service"),
			0,
			NULL,
...
void Service::deleteRegistryRecord() {
	HKEY hKey;
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Anti-virus service"),
		0, KEY_READ | KEY_WOW64_64KEY, &hKey) == ERROR_SUCCESS) {
		RegCloseKey(hKey);
		RegDeleteTree(HKEY_LOCAL_MACHINE, TEXT("SOFTWARE\\Anti-virus service"));
	}
}


void WINAPI Service::DoUpdateSvcDacl() {
	SC_HANDLE schSCManager;
	SC_HANDLE schService;
	EXPLICIT_ACCESS      ea;
...
TCHAR Service::WorkingDirectory[MAX_PATH];
